...
def post_to_instagram(username, password, image_path):
    cl = Client()
    cl.delay_range = [1, 3]  # To avoid rate limiting
    cl.set_locale('en_US')   # Just to be safe

    try:
        cl.load_settings('session.json')  # Load session if it exists
        cl.login(username, password)
    except Exception as e:
        print("Failed to load session, logging in fresh:", e)
        cl.login(username, password)

    if not cl.user_id:
        print("2FA required. Please provide the verification code.")
    ...
    try:
        cl.photo_upload(image_path, "Your inspirational quote here!")
